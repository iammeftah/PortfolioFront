...
export function ShootingStarsAndStarsBackgroundDemo() {
    ...
    const handleDownloadResume = async () => {
        try {
            const response: AxiosResponse<Blob> = await axios.get("http://localhost:8080/api/download-resume", {
                responseType: "blob",
            });
        ...
        <div className="h-screen bg-darkModeBg flex flex-col items-center justify-center relative w-full overflow-hidden">
          ...
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-purple-200 to-pink-300">
            MEFTAH Ahmed Reda
                ...
                <div className="flex flex-col md:flex-row items-center justify-center space-y-4 md:space-y-0 md:space-x-6 mt-8 scale-125">
                    <button
                        onClick={handleDownloadResume}
                        className="bg-slate-800 no-underline group cursor-pointer relative shadow-2xl shadow-zinc-900 rounded-full p-px text-xs font-semibold leading-6  text-white inline-block"
                    >
            <span className="absolute inset-0 overflow-hidden rounded-full">
              <span className="absolute inset-0 rounded-full bg-[image:radial-gradient(75%_100%_at_50%_0%,rgba(192,132,252,0.6)_0%,rgba(192,132,252,0)_75%)] opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
            </span>
